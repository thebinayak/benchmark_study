# Purpose: Functional Model for LSTM + MLP using Endmilling Audio and Force data
...
# Change two places: line number 24, 34 (for folder name ),38, 58, 168 and line 249(for optimizer)
# ============ CONSTANTS used in the program =====================
save_dir = os.path.join(os.getcwd(), 'LSTM_Sound+Force_MFCC') #RAdam AdamW
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

model_name = 'LSTM_MFCC_model'
maxlen = 130  # (orig = 323)
...
# Defind the function to load the Audio and Force data
...
def load_data(dataset_path):
    if dataset_path.endswith('.json'):
        ...
        Sound_input = np.array(data["mfcc"]) # mfcc or Mel_Spectrogram
...
def prepare_datasets(Sound_data, Force_data, Sound_label, Force_label, test_size, validation_size):
    ...
    Sound_train, Sound_test, Force_train, Force_test, \
    Sound_label_train, Sound_label_test, Force_label_train, Force_label_test = train_test_split(Sound_data,
                                                                                    Force_data,
                                                                                    Sound_label,
                                                                                    Force_label,
                                                                                    test_size=test_size)
...
def model_configure(Sound_data, Force_data):
    ...
    print(f'Sound Input shape is {Input_Layer_Sound.shape}')
    First = keras.layers.LSTM(64, return_sequences=True)(Input_Layer_Sound)
    Second = keras.layers.LSTM(64)(First)
    # Dense Layer
    Third = keras.layers.Dense(64, activation='relu')(Second)
    ...
    First = keras.layers.Dense(512, activation="relu", kernel_regularizer=keras.regularizers.l2(0.001))(
        Input_Layer_Force)  # First hidden layer
    Second = keras.layers.Dropout(0.3)(First)
    Third = keras.layers.Dense(256, activation="relu", kernel_regularizer=keras.regularizers.l2(0.001))(
        Second)  # Second hidden layey
    Fourth = keras.layers.Dropout(0.3)(Third)
    Fifth = keras.layers.Dense(64, activation="relu", kernel_regularizer=keras.regularizers.l2(0.001))(
        Fourth)  # Third hidden layer
    ...
    dot_img_file = os.path.join(save_dir,'LSTM_MFCC.png')
...
if __name__ == "__main__":
    for idx in range(len(random_states)):
        ...
        '''Call prepare_datasets() for CNN only
        S_train, S_validation, S_test, S_train_label, S_validation_label, S_test_label = prepare_datasets_CNN_sound(Sound_inputs,
                                                                                                  Sound_label,
                                                                                                  0.5,
                                                                                                  0.5)
    
        #X_train, X_validation, X_test, y_train, y_validation, y_test
        F_train, F_validation, F_test, F_train_label, F_validation_label, F_test_label = prepare_datasets_Force(Force_input,
                                                                                                  Force_label,
                                                                                                  0.5,
                                                                                                  0.5)
                                                                                                  '''
        ...
        Sound_label_train, Sound_label_validation, Sound_label_test, Force_label_train, Force_label_validation, Force_label_test = prepare_datasets(
            ...
            test_size=0.5,
            validation_size=0.5)
        '''# Further split test into test (50%) and validation(50%)
        Sound_test, Sound_validation, Force_test, Force_validation, \
        Sound_label_test, Sound_label_validation, Force_label_test, Force_label_validation = train_test_split(Sound_in_test,
                                                                                                              Force_in_test,
                                                                                                              Sound_label_test,
                                                                                                              Force_label_test,
                                                                                                              test_size=0.5)
...
test_loss, test_acc = model.evaluate([Sound_test, Force_test], Force_label_test, verbose=2)
...
disp.plot()
plt.savefig(os.path.join(save_dir, 'Confusion_Matrix_LSTM_MFCC.png'), dpi=300)
plt.show()
