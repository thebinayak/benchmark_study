...
save_dir = os.path.join(os.getcwd(), 'Transformer_MelSpectrogram') #RAdam AdamW
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

model_name = 'Transformer_Mel_Spectrogram' #AdamW
maxlen = 130
...
def load_data(dataset_path):
    if dataset_path.endswith('.json'):
        ...
        Sound_input = np.array(data["Mel_Spectrogram"]) #Mel_Spectrogram or mfcc
...
def model_configure(sound_train, force_train):
    ...
    # ================ CONSTANTS ==================
    # The shape of sound input is (1600, 130, 13) so the embed_dim_st = 13
    # The shape of force input is shape=(None, 130, 13)
    embed_dim_ST    = 132  # Embedding size for each token (n_fft/2)+1 =(2048/2)+1 =  1025 (orig = 2049) 15-->128
    ...
    dot_img_file = os.path.join(save_dir, 'Transformer_Mel_Spectrogram.png')
...
disp.plot()
plt.savefig(os.path.join(save_dir, 'Confusion_Matraix.png'), dpi=300)
plt.show()
