...
# Change two places: line number 34 (for folder name ),38, 58, 168 and line 249(for optimizer)
# ============ CONSTANTS used in the program =====================
save_dir = os.path.join(os.getcwd(), 'LSTM_Sound+Force_MelSpectrum') #RAdam AdamW
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

model_name = 'LSTM_MelSpectrum_model'
maxlen = 130
...
def load_data(dataset_path):
    if dataset_path.endswith('.json'):
        ...
        Sound_input = np.array(data["Mel_Spectrogram"]) # mfcc or Mel_Spectrogram
...
def model_configure(Sound_data, Force_data):
    ...
    dot_img_file = os.path.join(save_dir,'LSTM_MelSpectrum.png')
...
disp.plot()
plt.savefig(os.path.join(save_dir, 'Confusion_Matrix_LSTM_Mel_Spectrogram.png'), dpi=300)
plt.show()
